...
def process_and_normalize_hotel_data():
    try:
        ...
        reviews_df = pd.read_csv("../dat/cleaned_reviews.csv", encoding="utf-8-sig")
        ...
        combined_df = pd.merge(reviews_df, offerings_df, on="offering_id", how="inner")

        # # Process JSON columns in combined_df
        # combined_df["ratings"] = combined_df["ratings"].str.replace("'", '"')
        # combined_df["ratings"] = combined_df["ratings"].apply(json.loads)
        # normalized_cols = pd.json_normalize(combined_df["ratings"])
        # combined_df = combined_df.join(normalized_cols).drop(columns=["ratings"])

        # # Apply text preprocessing steps to the 'text' column in combined_df
        ...
        # )
        # combined_df["text"] = combined_df["text"].apply(lambda text: remove_urls(text))
        ...
        sorted_lexicon = {
            token: idx + 1 for idx, token in enumerate(sorted_lexicon_keys)
        }
